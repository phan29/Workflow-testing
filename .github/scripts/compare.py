...
def generate_size_diff_html(build_variant, base_apk_name, head_apk_name, start_index):
    global html
    ...
    html += f"<li><h4><code>{build_variant}</code></h4><table>"
    html += f"<tr><th>Component</th><th>Base ({base_apk_name})</th><th>Head ({head_apk_name})</th><th>Diff</th><th>Change in Percentage</th></tr>"
    ...
    for component in set(base_apk_components.keys()) | set(head_apk_components.keys()):
        ...
        html_data.append([component, format_size(base_apk_size), format_size(head_apk_size), format_size_with_indicator(head_apk_size - base_apk_size), get_diff_in_percentage(head_apk_size - base_apk_size, base_apk_size)])

    order = ["Classes", "Assets", "Resources", "Native libraries (x86)", "Native libraries (x86_64)", "Native libraries (arm64-v8a)", "Native libraries (armeabi-v7a)", "Others"]
    ...
    html += f"<tr><th>APK (Upload Size)</th><th>{format_size(base_apk_download_size)}</th><th>{format_size(head_apk_download_size)}</th><th>{format_size_with_indicator(head_apk_download_size - base_apk_download_size)}</th><th>{get_diff_in_percentage(head_apk_download_size - base_apk_download_size, base_apk_download_size)}</th></tr>"
    html += "</table></li>"
...
def format_size_with_indicator(size):
    ...
    return f"{abs(float(converted_size[0])) if float(converted_size[0]) >= 0 else converted_size[0]} {converted_size[1]} {change_indicator}"
